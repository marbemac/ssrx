import type { AnyRouteMatch, AnyRouter, ExtractedEntry, StreamState } from '@tanstack/react-router';
import {
  ...
  router.state.matches.forEach(match => {
    ...
    if (dMatch) {
      ...
      if (dMatch.__beforeLoadContext) {
        match.__beforeLoadContext = router.options.transformer.parse(dMatch.__beforeLoadContext) as any;
      ...
      if (extracted) {
        Object.entries(extracted).forEach(([, ex]: any) => {
...
export function AfterEachMatch(props: { match: any; matchIndex: number }) {
  ...
  const extracted = (fullMatch as any).extracted as undefined | Array<ExtractedEntry>;

  const [serializedBeforeLoadData, serializedLoaderData] = (['__beforeLoadContext', 'loaderData'] as const).map(
    dataType => {
      return extracted
        ? extracted.reduce(
  ...
  return (
    <>
      {serializedBeforeLoadData !== undefined || serializedLoaderData !== undefined || extracted?.length ? (
        <ScriptOnce
          children={`__TSR__.matches[${props.matchIndex}] = ${jsesc(
            {
              __beforeLoadContext: router.options.transformer.stringify(serializedBeforeLoadData),
...
export function replaceBy<T>(obj: T, cb: (value: any, path: Array<string>) => any, path: Array<string> = []): T {
  if (isPlainArray(obj)) {
...
function InnerDehydratePromise({ entry }: { entry: ExtractedEntry }) {
  ...
  return (
    <ScriptOnce
      children={`__TSR__.matches[${entry.matchIndex}].extracted[${entry.id}].resolve(${jsesc(entry.value.data, {
        isScriptContext: true,
        wrap: true,
        json: true,
      })})`}
    ...
    <StreamChunks
      ...
      children={chunk => (
        <ScriptOnce
          children={
            chunk
              ? `__TSR__.matches[${entry.matchIndex}].extracted[${
                  entry.id
                }].value.controller.enqueue(new TextEncoder().encode(${jsesc(chunk.toString(), {
                  isScriptContext: true,
                  wrap: true,
                  json: true,
                })}))`
...
function StreamChunks({
  ...
  return (
    <>
      ...
      <div className="tsr-once">
        <React.Suspense fallback={null}>
          <StreamChunks streamState={streamState} __index={__index + 1} children={children} />
        </React.Suspense>
      </div>
    </>
  );
